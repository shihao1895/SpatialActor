...
from spatial_actor.models.augmentation.augmentation import apply_se3_aug_con
import spatial_actor.models.augmentation.aug_utils as aug_utils
import spatial_actor.utils.constant as constant
...
def comp_align_loss(ft_t, ft_s):
    """
    spatial+channel align loss
    inputs:
        ft_t: (b, c, h, w) teacher feature
        ft_s: (b, c, h, w) student feature
    outputs:
        loss
    """
    # Spatial attn align
    spa_t = torch.mean(ft_t, dim=1, keepdim=True)
    spa_s = torch.mean(ft_s, dim=1, keepdim=True)
    spa_loss = F.mse_loss(spa_s, spa_t)

    # Channel attn align
    chn_t = torch.mean(ft_t, dim=(2, 3))  # (b, c)
    chn_s = torch.mean(ft_s, dim=(2, 3))  # (b, c)

    chn_t = F.normalize(chn_t, dim=1)
    chn_s = F.normalize(chn_s, dim=1)

    temperature = 0.07
    similarity_matrix = torch.mm(chn_t, chn_s.t())  # (b, b)
    similarity_matrix = similarity_matrix / temperature

    # generate target labels: positive samples are on the diagonal
    labels = torch.arange(similarity_matrix.size(0)).to(ft_t.device)  # (b,)

    # InfoNCE
    chn_loss = F.cross_entropy(similarity_matrix, labels)

    loss = spa_loss + chn_loss
    return loss


def add_noise_to_pointcloud(pointcloud, noise_type):
    noise_ratio = constant.NOISE_CFG[noise_type]['ratio']
    noise_std = constant.NOISE_CFG[noise_type]['std']

    pc_min = torch.min(pointcloud, dim=1, keepdim=True).values  # (b, 1, c)
    pc_max = torch.max(pointcloud, dim=1, keepdim=True).values  # (b, 1, c)
    pc_range = pc_max - pc_min  # (b, 1, c)

    # channel-wise std (b, 1, c)
    noise_std = pc_range * noise_std

    # randomly select points to add noise
    b, n, c = pointcloud.shape
    noise_mask = torch.rand((b, n), device=pointcloud.device) < noise_ratio  # (b, n)
    noise_mask = noise_mask.unsqueeze(-1)  # (b, n, 1)

    # generate noise
    noise = torch.randn_like(pointcloud) * noise_std  # (b, n, c)
    noisy_pointcloud = pointcloud + noise * noise_mask
...
class Agent:
    def __init__(
        ...
        scene_bounds: list = constant.SCENE_BOUNDS,
        cameras: list = constant.CAMERAS,
        rot_x_y_aug: int = 2,
        log_dir="",
        noise_type='none',
        **kwargs,
        ...
        :param rot_x_y_aug: it specifies how
    ...
    def update(
        ...
        lang = replay_sample["lang_goal"]
        tasks = replay_sample["tasks"]

        proprio = agent_utils.stack_on_channel(replay_sample["low_dim_state"])  # (b, 4)
        return_out = {}

        obs, pcd = agent_utils.preprocess_inputs(replay_sample, self.cameras)

        with torch.no_grad():
            pc, img_feat = agent_utils.get_pc_img_feat(
                obs,
                pcd,
            ) # (b,n,c)

            if self.noise_type != 'none':
                pc = add_noise_to_pointcloud(
                    pc, noise_type=self.noise_type,
                )
    ...
    def act(
        ...
        lang_goal_tokens = observation.get("lang_goal_tokens", None).long()[0]

        proprio = agent_utils.stack_on_channel(observation["low_dim_state"])

        lang = observation["lang_goal"]

        obs, pcd = agent_utils.preprocess_inputs(observation, self.cameras)
        pc, img_feat = agent_utils.get_pc_img_feat(
            obs,
            pcd,
        )
    ...
    def get_pred(
        ...
        pred_wpt_local = self._net_mod.get_wpt(
            out, spact_1_or_2, dyn_cam_info, y_q
        )
    ...
    def get_action_trans(
        ...
        action_trans = model_utils.generate_hm_from_pt(
            wpt_img.reshape(-1, 2),
            (h, w),
